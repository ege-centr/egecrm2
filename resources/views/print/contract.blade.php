<div class="print print__contract">
    ...
    <div style="width: 100%; margin-bottom: 20px">
        <div>г. Москва, {{ $contractDate }} г.</div>
    </div>

    <p>
        <b>Общество с ограниченной ответственностью «ЕГЭ-ЦЕНТР»</b>, в лице Генерального директора Эрдмана Константина Александровича,
        действующего на основании Устава, именуемое в дальнейшем «Исполнитель», «ЕГЭ-Центр», на основании лицензии на осуществление образовательной
        деятельности № 037742, выданной Департаментом образования города Москвы 08.08.2016 г., с одной стороны, и
        {{ $contract->client->representative->names->full }}, именуемый(ая)
        в дальнейшем «Заказчик», являющийся(щаяся) родителем (законным представителем)
        @name($contract->client->names->full, 'родительный'), {{ date('d.m.Y', strtotime($contract->client->birthdate)) }} года рождения, именуемого(ой)
        в дальнейшем «Обучающийся», с другой стороны, совместно именуемые «Стороны», заключили настоящий Договор на оказание платных образовательных услуг
        (далее – «Договор») о нижеследующем:
    </p>

    <h4>1. Предмет Договора.</h4>
    <p>
        1.1. По настоящему Договору Исполнитель обязуется зачислить Обучающегося на обучение и оказать Обучающемуся образовательные услуги по программе дополнительного обучения по курсу (курсам):
        @foreach($contract->subjects as $subject)
            «{{ $subject->subject->name }}» (@plural($subject->lessons, 'занятие')){{ $loop->last ? '' : ',' }}
        @endforeach
        (далее – «Услуги»), а Заказчик обязуется оплатить указанные Услуги в порядке и на условиях, которые установлены настоящим Договором.
    </p>

    <p>1.2. Обучение осуществляется в порядке, установленном локальными нормативными актами Исполнителя и настоящим Договором.</p>

    <p>
        1.3. Продолжительность образовательной программы по программе курса
        @foreach($contract->subjects as $subject)
            «{{ $subject->subject->name }}–{{ $contract->grade->short }}–{{ $subject->lessons_planned }}»
            (@plural(ceil($subject->lessons_planned * 3), 'ауд. час') и
            @plural(ceil($subject->lessons_planned * 1.5), 'час') на самостоятельную подготовку){{ $loop->last ? '.' : ',' }}
        @endforeach
    ...
    <p>1.5. Вид образования: дополнительное образование детей и взрослых</p>
    ...
    <p><b>2.1. Исполнитель обязуется:</b></p>
        ...
        <ul style="margin: 0">
            ...
            <li>Логин: email Заказчика, указанный при заключении Договора ({{ optional($contract->client->representative->email)->email ?? 'email отсутствует' }})</li>
        ...
        <ul style="margin: 0">
            <li>Адрес: https://lk.ege-centr.ru</li>
            <li>Логин: email Обучающегося, указанный при заключении Договора ({{ optional($contract->client->email)->email ?? 'email отсутствует' }})</li>
    ...
    <p>2.1.10. Провести обучение по программе до начала основного периода сдачи государственных экзаменов по выбранным дисцимлинам</p>
    ...
    <p>3.1.
        Общая стоимость Услуг Исполнителя по Договору складывается из стоимости @plural(collect($contract->subjects)->sum('lessons'), 'занятие'),
        приобретаемых на момент заключения Договора, и
        @if($contract->discount > 0)
            с учетом скидки {{ $contract->discount }}%
        @endif составляет {{ $contract->discounted_sum }} ({{ numToText($contract->discounted_sum) }}) руб.
    </p>

    <p>3.2.
    @if($contract->discount > 0)
    Стоимость одного занятия с учетом скидки {{ $contract->discount }}%
    @else
    Стоимость одного занятия
    @endif
    составляет {{ $oneSubjectPrice }} руб.
    </p>
    ...
    <p>3.4. Оплата Услуг по настоящему Договору
        @if($contract->discount > 0)с учетом скидки {{ $contract->discount }}%@endif
        производится Заказчиком следующим образом:
        <ul style='margin: 0'>
            @foreach($contract->payments as $payment)
            <li>
                {{ count($contract->payments) === 1 ? 'единовременный' : '' }} платеж в размере {{ $oneSubjectPrice * $payment->lessons }} руб.  (@plural($payment->lessons, 'занятие')) производится
                {{ $loop->first ? 'при заключении Договора' : 'до ' . date('d.m.Y', strtotime($payment->date)) }}
            </li>
            @endforeach
        </ul>
    ...
    Согласие на обработку моих персональных данных и персональных данных Обучающегося  предоставляю: </p>

    <p style='margin-top: 40px'>___________________________________________________________________ </p>
    ...
    <li>по инициативе Заказчика при условии письменного уведомления второй Стороны не позднее, чем за 10 (десять) рабочих дней;</li>
    <li>по инициативе Исполнителя при условии уведомления второй Стороны не позднее, чем за 5 (пять) рабочих дней;</li>
    ...
    <ul>
        <li>если Исполнитель отказывается от исполнения Договора, то он возвращает Заказчику все полученные от него в порядке предварительной оплаты на основании соответствующей заявки на обучение денежные средства, за исключением стоимости фактически оказанных услуг.</li>
        <li>если Заказчик отказывается от исполнения Договора, то он обязан оплатить оказанные услуги (состоявшиеся занятия) к моменту отказа от Договора.</li>
        <li>односторонний отказ от исполнения Договора любой из Сторон по основаниям, указанным в настоящем Договоре, влечет за собой его прекращение во внесудебном порядке.</li>
    </ul>
    ...
    <p>6.5.  В случае одностороннего отказа Исполнителя от исполнения договора на основании п.п 6.3., Заказчик считается уведомленным если уведомление направлено любым возможным способом (почта, электронная почта, sms-уведомление, телефонограмма и другими). </p>
    ...
    <p>Стороны будут прилагать все усилия к тому, чтобы решать возникающие разногласия путем переговоров. В случае возникновения разногласий и споров, связанных с исполнением настоящего Договора, Стороны принимают усилия по их урегулированию путем переговоров. Все претензии подаются только в письменной форме и должны быть рассмотрены в течение 30 (тридцати) дней. Если Стороны не достигнут компромисса, спор подлежит разрешению в судебном порядке в соответствии с правилами процессуального законодательства РФ.</p>
    ...
    <p>11.2. Настоящий Договор вступает в силу с даты его подписания, заключен на срок до 01 июля {{ $contract->year + 1 }} года.</p>
    ...
    <p>11.4. Вся переписка между Сторонами, в том числе обязательные документы, связанные с исполнением настоящего Договора, направляются путем использования почты, электронной почты, курьерскими службами и/или иными, установленными законом способами. Корреспонденция считается полученной, если она была направлена:</p>
        <ul>
            <li>По электронной почте на адрес, указанный Сторонами в разделе 12 настоящего Договора;</li>
            <li>По почте на фактический адрес, указанный Сторонами в разделе 12 настоящего Договора и в соответствии с правилами Почты России; </li>
            <li>Курьерской службой – в соответствии с правилами отправки корреспонденции выбранной курьерской службой;</li>
            <li>В иных случаях – когда имеется отметка о принятии корреспонденции уполномоченным лицом.</li>
        </ul>

    <p>11.5. Подписание настоящего Договора является подтверждением того, что Заказчик ознакомлен с локальными актами и иными документами и информацией  Исполнителя, размещенными на сайте в сети Интернет http://www.ege-centr.ru/ в соответствии с требованиями ФЗ "Об образовании".</p>
    <p>11.6. Приложение №1 является неотъемлемой частью Договора. </p>
    ...
    <div style="display: inline-block; width: 100%">
                <div style="width: 100%">
                    ...
                    ИНН 9701038111<br>
                    г. Москва, ул. Мясницкая, д. 40, <br>
                    стр. 1, 3 эт., к. 7, 101000<br>
                ...
                <div style="width: 100%; margin-top: 50px">
                    <b>ЗАКАЗЧИК:</b><br>
                    {{ $representative->names->full }}<br>
                    Паспорт {{ $representative->series}} номер {{ $representative->number}}<br>
                    ...
                    e-mail: {{ $representative->email->email }}<br>
                    @endif

                    <div style='margin-top: 30px'>
                        {{ $representative->names->abbreviation }}
                        ________________________________________________________
                    </div>
                </div>
            </div>
    ...
    №{{ $contract->number }} от {{ $contractDate }}
    ...
    <div style="margin-bottom: 20px">
        г. Москва, «____»_______ 20____ г.
    </div>
